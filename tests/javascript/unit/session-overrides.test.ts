...
describe('session-overrides.ts', () => {
  ...
  describe('edge cases', () => {
    it('should handle sessions directory does not exist', () => {
      const result = loadSessionOverrides(carlDir, 'no-sessions');
  ...
  describe('helper functions', () => {
    ...
    describe('getDomainOverride', () => {
      it('should return override value for domain', () => {
        ...
        expect(getDomainOverride('DEVELOPMENT', overrides)).toBe('false');
        expect(getDomainOverride('CONTENT', overrides)).toBe('true');
        expect(getDomainOverride('DATABASE', overrides)).toBe('inherit');
      });
  ...
  describe('edge cases', () => {
    ...
    it('should handle .carl directory does not exist', () => {
      const nonExistentCarlDir = path.join(tempDir, 'non-existent');
    ...
    it('should handle non-object domains field', () => {
      const sessionsDir = path.join(carlDir, 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.writeFileSync(
        path.join(sessionsDir, 'invalid-domains.json'),
        JSON.stringify({ domains: 'not-an-object' })
      );

      const result = loadSessionOverrides(carlDir, 'invalid-domains');

      expect(result.exists).toBe(true);
      expect(result.domains).toEqual({});
    });

    it('should handle missing domains field', () => {
      const sessionsDir = path.join(carlDir, 'sessions');
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.writeFileSync(
        path.join(sessionsDir, 'no-domains.json'),
        JSON.stringify({ otherField: 'value' })
      );

      const result = loadSessionOverrides(carlDir, 'no-domains');
